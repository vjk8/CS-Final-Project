...
public class ImageProcessing {
    ...
    /**
     * Copied from AthleteOCR for debugging purposes
    ...
    /**
     * Rotates an image by a certain angle, with positive taken to be counterclockwise
     * @param img the image
    ...
    public static Mat deskew(Mat img) {
        ...
        try {
            if (corners[0][0] * 2 >= img.rows()) {
                ...
                return rotate(img,
                              -Math.atan(((double)corners[2][1] - corners[1][1]) / (corners[2][0] - corners[1][0])) /
                                  Math.PI * 180);
            }
            // Probably skewed counterclockwise
            return rotate(img, -Math.atan(((double)corners[1][1] - corners[0][1]) / (corners[1][0] - corners[0][0])) /
                                   Math.PI * 180);
