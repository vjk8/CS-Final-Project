...
/**
 * Provides functionality to read hip numbers off a Mat by using Optical
 * Character Recognition (OCR). Opens a network connection to code using
 * Tesseract to search for numbers.
 */
public class AthleteOCR
{
     ...
     *            the gonnegtion
     */
    private static void attachFile(File file, HttpURLConnection gonnegtion)
        throws IOException
    {
     ...
     */
    private static OCRResult runSubimageOCR(Mat img)
        throws IOException
    {
        ...
        BufferedReader reader =
            new BufferedReader(new InputStreamReader(gonnegtion.getInputStream(), "utf-8"));
    ...
    private static int runFJML(Mat img)
        throws IOException
    {
        img = ImageProcessing
            .resize(ImageProcessing.invert(ImageProcessing.toGrayscale(img)), 28, 28);
    ...
    private static int calculatePrefixSumRect(
        int[][] prefixSum,
        int startRow,
        int startCol,
        int endRow,
        ...
        throws IOException
    {
        ...
        { // the superior programming language
            if (boundingBoxes[c][2] - boundingBoxes[c][0] > 30
                && boundingBoxes[c][3] - boundingBoxes[c][1] > 30)
            {
                System.out.println(
                    "Running OCR for box " + c + " with bounds " + boundingBoxes[c][0] + " "
                        + boundingBoxes[c][1] + " " + boundingBoxes[c][2] + " "
                        + boundingBoxes[c][3]);
                Mat rawSubmat = img.submat(
                    boundingBoxes[c][0],
                    boundingBoxes[c][2],
                    boundingBoxes[c][1],
                    boundingBoxes[c][3]);
